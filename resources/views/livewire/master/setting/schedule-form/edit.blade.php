...
use App\Livewire\Forms\ScheduleFormForm;
use Livewire\Volt\Component;

new class extends Component {
    public ScheduleFormForm $form;
...
<div>
    <form wire:submit='save'>
        ...
        <div class="mt-4">
            ...
            <x-text-input id="start" class="block w-full mt-1 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" type="date" name="start" :value="old('start')"
                wire:model='form.start' />
            <x-input-error for="start" class="mt-2" />
        </div>
        <div class="mt-4">
            <x-input-label for="end" value="{{ __('End') }}" />
            <x-text-input id="end" class="block w-full mt-1 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" type="date" name="end" :value="old('end')"
                wire:model='form.end' />
            <x-input-error for="end" class="mt-2" />
        </div>
        <div class="mt-4">
            <label class="relative inline-flex items-center mb-5 cursor-pointer">
                <input type="checkbox" class="sr-only peer" wire:model='form.status'>
