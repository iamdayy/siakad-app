<div x-data="{ open: {{ isset($open) && $open ? 'true' : 'false' }}, working: false }" x-cloak wire:key="delete-{{ $value->title ? $value->title : $value->username }}">
    ...
    <div x-show="open"
        ...
        <div x-show="open" x-transition:enter="ease-out duration-300"
            ...
            class="relative bg-primary dark:bg-primary-dark rounded-lg px-4 pt-5 pb-4 overflow-hidden shadow-xl transform transition-all sm:max-w-lg sm:w-full sm:p-6">
            <div class="hidden sm:block absolute top-0 right-0 pt-4 pr-4">
                <button @click="open = false" type="button"
                    class="text-secondary dark:text-secondary-dark hover:text-secondary-dark hover:dark:text-secondary focus:outline-none focus:text-gray-500 transition ease-in-out duration-150">
            ...
            <div class="w-full">
                <div class="mt-3 text-center">
                    <h3 class="text-lg leading-6 font-medium  text-primary-dark dark:text-primary">
                        {{ __('Delete') }} {{ $value->title ? $value->title : $value->username }}
                    </h3>
                    <div class="mt-2">
