<div>
    ...
    <div class="relative">
                    ...
                    <div class="flex rounded-lg w-96 shadow-sm">
                        <div class="relative flex-grow focus-within:z-10">
                            ...
                            <input wire:model.debounce.500ms="search"
                                class="block w-full py-3 pl-10 text-sm bg-primary dark:bg-primary-dark border-gray-300 leading-4 rounded-md shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50 focus:outline-none"
            ...
            <div class="flex flex-wrap items-center space-x-1">
                ...
                @if ($this->activeFilters)
                    <button wire:click="clearAllFilters"
                        class="flex items-center px-3 text-xs font-medium tracking-wider text-red-500 uppercase bg-primary dark:bg-primary-dark border border-red-400 space-x-2 rounded-md leading-4 hover:bg-red-200 focus:outline-none"><span>{{ __('Reset') }}</span>
                ...
                @if (count($this->massActionsOptions))
                    <div class="flex items-center justify-center space-x-1">
                        ...
                        <select wire:model.live="massActionOption"
                            class="px-3 text-xs font-medium tracking-wider uppercase bg-primary dark:bg-primary-dark border border-green-400 space-x-2 rounded-md leading-4 focus:outline-none"
                        ...
                        <button wire:click="massActionOptionHandler"
                            class="flex items-center px-4 py-2 text-xs font-medium tracking-wider text-green-500 uppercase bg-primary dark:bg-primary-dark border border-green-400 rounded-md leading-4 hover:bg-green-200 focus:outline-none"
                ...
                @if ($exportable)
                    <div x-data="{
                        ...
                        <button wire:click="export"
                            class="flex items-center px-3 text-xs font-medium tracking-wider text-green-500 uppercase bg-primary dark:bg-primary-dark border border-green-400 space-x-2 rounded-md leading-4 hover:bg-green-200 focus:outline-none"><span>{{ __('Export') }}</span>
                ...
                @foreach ($columnGroups as $name => $group)
                    <button wire:click="toggleGroup('{{ $name }}')"
                        class="px-3 py-2 text-xs font-medium tracking-wider text-green-500 uppercase bg-primary dark:bg-primary-dark border border-green-400 rounded-md leading-4 hover:bg-secondary hover:dark:bg-secondary-dark focus:outline-none">
                        <span
            ...
            <div class="p-2 grid grid-cols-8 gap-2">
                @foreach ($this->columns as $index => $column)
                    @if ($column['hideable'])
                        ...
                        {{ $column['hidden'] ? 'bg-primary dark:bg-primary-dark hover:bg-blue-300 text-blue-600' : 'bg-blue-500 hover:bg-blue-800' }}">
                            {{ $column['label'] }}
                        </button>
                    @endif
                @endforeach
        ...
        <div wire:loading.class="opacity-50"
            ...
            <div>
                <div class="table min-w-full align-middle">
                            ...
                            @foreach ($this->columns as $index => $column)
                                ...
                                @elseif($column['type'] === 'checkbox')
                                    @unless ($column['hidden'])
                                        <div
                                            class="flex justify-center table-cell w-32 h-12 px-6 py-4 overflow-hidden text-xs font-medium tracking-wider text-left text-gray-500 uppercase align-top border-b  border border-secondary dark:border-secondary-dark bg-primary dark:bg-primary-dark leading-4 focus:outline-none">
                                            <div
                                                class="px-3 py-1 rounded @if (count($selected)) bg-orange-400 @else bg-primary dark:bg-primary-dark @endif text-white text-center">
                    ...
                    @endunless
                    <div class="table-row bg-primary dark:bg-primary-dark">
                        @foreach ($this->columns as $index => $column)
                            @if ($column['hidden'])
                                @if ($hideable === 'inline')
                                    <div
                                        class="table-cell w-5 overflow-hidden align-top bg-primary dark:bg-primary-dark">
                                    </div>
                                @endif
                            @elseif($column['type'] === 'checkbox')
                ...
                <div class="items-center justify-between p-2 sm:flex">
                    ...
                    @if (count($this->results))
                        <div class="flex items-center my-2 sm:my-0">
                            <select name="perPage"
                                class="block w-full py-2 pl-3 pr-10 mt-1 text-base bg-primary dark:bg-primary-dark text-primary-dark dark:text-primary form-select leading-6 rounded-lg sm:text-sm sm:leading-5"
                        ...
                        <div class="flex justify-end text-secondary-dark dark:text-secondary">
        ...
        @if ($complex)
            <div
                class="bg-primary dark:bg-primary-dark px-4 py-4 rounded-b-lg rounded-t-none shadow-lg border-2 @if ($this->activeFilters) border-blue-500 @else border-transparent @endif @if ($complex) border-t-0 @endif">
        ...
        <span
            class="hidden text-sm text-left text-center text-right text-gray-900 bg-primary dark:bg-primary-dark bg-yellow-100 leading-5"></span>
    </div>
